...
export const calculateBadges = (totalKg, rank, friendsCount, cityRank, householdSize = 1, sensorCount = 0, lastThrowDate = null, streakDays = 0) => {
  ...
  // 1. BADGE AMIS (Réseau) : 1 -> 3 -> 10 amis
  ...
  // 2. BADGE POIDS TOTAL (Limitation) : <15kg -> <5kg -> 0kg
  ...
  // 3. BADGE MINIMALISTE (Ratio par personne) : <5kg -> <3kg -> <1kg
  ...
  // 4. BADGE COMPÉTITEUR (Classement Amis) : Top 50% -> Top 3 -> 1er
  ...
  // 5. BADGE VILLE (Classement Local) : Top 10 -> Top 3 -> 1er
  ...
  // 7. BADGE FLAMME (Streak : jours consécutifs < 1kg/pers)
