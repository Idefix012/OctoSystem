...
const CommunityView = ({ user }) => {
  ...
  useEffect(() => {
    ...
    const fetchCommunityData = async () => {
      try {
        ...
        const repRequests = await fetch(`${API_URL}/friends/requests`, {
        ...
        const repLeaderboard = await fetch(`${API_URL}/friends`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        ...
        const repStreak = await fetch(`${API_URL}/user/streak`, {
          method: 'GET',
          headers: { 'Authorization': `Bearer ${token}` }
        });
        if (repStreak.ok) {
  ...
  useEffect(() => {
    ...
    const fetchCityLeaderboard = async () => {
      ...
      try {
        const token = localStorage.getItem('octo_token');
        const response = await fetch(`${API_URL}/city/leaderboard?period=${timeframe}`, {
          method: 'GET',
          headers: { 'Authorization': `Bearer ${token}` }
        });
        if (response.ok) {
  ...
  useEffect(() => {
    ...
    const syncBadges = async () => {
      ...
      try {
        ...
        const rep = await fetch(`${API_URL}/badges/owned`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        ...
        for (const badge of badgesMerites) {
          ...
          if (badge.isPermanent && badge.unlocked && !dbBadges.includes(badge.id)) {
            const unlockRep = await fetch(`${API_URL}/badges/unlock`, {
              method: 'POST',
  ...
  const handleManageRequest = async (friendCode, action) => {
    try {
      const token = localStorage.getItem('octo_token');
      const response = await fetch(`${API_URL}/friends/manage`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
